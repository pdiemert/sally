...
function LoadRunner(test, options)
{
    ...
    function onCmd_workerMetrics(data)
    {
        ...
        if (_.keys(_workermetrics).length == _workerCount)
        {
            ...
            // Merge clocks
            for(var c in data.clock)
            {
                var clock = mClock[c];
                if (!clock)
                {
                    clock = {count:0,elapsed:0,avg:0};
                    mClock[c] = clock;
                }

                clock.count += data.clock[c].count;
                clock.elapsed += data.clock[c].elapsed;
                clock.avg = clock.elapsed / clock.count;
            }
