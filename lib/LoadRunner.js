...
function LoadRunner(test, options)
{
	...
	function runProfile()
	{
		...
		_wpop = new WindowPop('Population', _maxpop, _startTime, _options.duration);
	}
...
function WindowPop(title, maxpop, start, dursec)
{
	var _dur = dursec * 1000;
	var _startTime = start;
	var _max = maxpop;
	var _w = new p_cur.Window(Math.round(_wscr.height * 0.5),_wscr.width);
	...
	_w.move(Math.round(_w.height * 0.1), 0);
