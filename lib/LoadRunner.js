...
function LoadRunner(test, options)
{
	...
	function onCmd_workerMetrics(data)
	{
		...
		if (_.keys(_workermetrics).length == _workerCount)
		{
			...
			if (_lastts)
			{
				var d = Date.now() - _lastts;
				var ds = totals - _lasts;

				var transec = Math.round(ds / (d / 1000));

				_wtrans.refresh(transec.toFixed(1) + '/s');
			}
	...
	function runProfile()
	{
		...
		_wpopc = new GaugeWidget('Population', _wpop.bottom + 1, _wfail.right + 2);
		_wtrans = new GaugeWidget('Transactions', _wpop.bottom + 1, _wpopc.right + 2, 9);
	}
...
function StaticHistWidget(title, maxpop, start, dursec, r, c, w, h)
{
	...
	strat(_w, _w.height - _padb, _padl-1, '+');
	hbar(_w, _w.height - _padb, _padl, _w.width - (_padl + _padr), '-');

	strat(_w, 0, 0, title);
	strat(_w, _padt, _padl-2, _maxpop, { right : true});
	strat(_w, _w.height-1, _w.width, (_dur / 1000).toFixed(0) + 's', {right : true});
...
function GaugeWidget(title, r, c, w)
{
	var _val = 0;
	var _w = new p_cur.Window(3, title.length + (w || 5) + 6);
	this.window = _w;
	...
	_w.box();
	strat(_w, 1, 2, title + ':');
	strat(_w, 1, _w.width-2, 0, { right : true});
	...
	{
		_val = v;
		strat(_w, 1, _w.width-2, _val, { right : true});
