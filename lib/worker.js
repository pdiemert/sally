...
function VirtualUser(user, index)
{
    ...
    this.start = function ()
    {
        ...
        p_ssa.runSuite(suite, opt, handleSuiteFinish);
    };
...
{
    if (!('repeatDelay' in g_test.options)) return p_const.defaultRepeatDelay;

    if (_.isNumber(g_test.options.repeatDelay)) return g_test.options.repeatDelay;

    if (!('user' in g_test.options.repeatDelay)) return p_const.defaultRepeatDelay;
...
function updateWorkers()
{
    ...
    curSave();
    puts('Test running, ' + (g_test.options.duration - elapsed) + 's remaining   ');
    curLoad();

    // If we've hit duration then finish
    if (elapsed >= g_test.options.duration) finish();
    else
    {
...
function pollMetrics()
{
    getMetrics(function(m)
    {
        ...
        _.keys(g_pop).forEach(function(e) { pop[e] = g_pop[e].length;  });
        _.keys(g_userstat).forEach(function(e) { stat[e] = {s:g_userstat[e].s, f:g_userstat[e].f, a:g_userstat[e].a };});
