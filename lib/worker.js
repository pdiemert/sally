...
function finish()
{
	...
	var spindown = g_test.options.spindown === undefined ? 60 : g_test.options.spindown;

	var startwait = new Date().getTime();
	var wait = setInterval(function()
	{
		var sec = Math.floor((new Date().getTime() - startwait) / 1000);
		if (g_suiteConcurrent > 0 && sec < spindown)
		{
			//curSave();
			//puts('Waiting ' + (spindown-sec)  +  ' more second(s) for ' + g_suiteConcurrent + ' suite(s) to complete...                       \r');
			//curLoad();
		}
		else
		{
...
function pollMetrics()
{
	...
	for(var clockname in g_reqclock)
	{
		var clock = g_reqclock[clockname];

		if (!clock.hist)
			clock.hist = [];

		var offset = new Date().getTime() - g_starttime.getTime();

		clock.hist.push({offset:offset,count:clock.count,elapsed:clock.elapsed,avg:clock.avg});
	}
